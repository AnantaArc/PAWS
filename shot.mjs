...
async function capture(theme) {
  ...
  // force the theme we want, then reload so every token applies
  await page.evaluate((th) => localStorage.setItem("paws-theme-v2", th), theme);
  await page.reload({ waitUntil: "networkidle" });
  await page.waitForTimeout(4500);

  await page.screenshot({ path: `/home/user/qa-screens/${theme}-top.png` });
  await page.evaluate(() => window.scrollBy(0, 900));
  await page.waitForTimeout(700);
  await page.screenshot({ path: `/home/user/qa-screens/${theme}-mid.png` });
  await page.close();
}
